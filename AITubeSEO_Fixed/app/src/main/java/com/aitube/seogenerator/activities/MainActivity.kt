...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ads init is wrapped in try/catch — will NEVER crash the app
        initAdsSafely()
        setupClickListeners()
        observeViewModel()
        updateLimitUI()
    ...
    // ── AdMob — 100% CRASH-FREE implementation ───────────────

    /**
     * ROOT CAUSE OF CRASH WITH REAL APP IDs:
     *
     *  1. The APPLICATION_ID in AndroidManifest must exactly match your
     *     AdMob App ID (format: ca-app-pub-XXXXXXXXXXXXXXXX~XXXXXXXXXX).
     *
     *  2. MobileAds.initialize() must complete BEFORE any ad is loaded.
     *     We solve this by loading ads only inside the init callback.
     *
     *  3. Never call ad.show() after Activity is finishing/destroyed.
     *     All show() calls are guarded by isFinishing/isDestroyed checks.
     *
     *  4. The DELAY_APP_MEASUREMENT_INIT meta-data in the manifest
     *     prevents AdMob from crashing during Application startup.
     *
     * HOW TO SWITCH TO YOUR REAL IDs:
     *  - Open res/values/strings.xml
     *  - Replace admob_app_id    → your real App ID
     *  - Replace admob_banner_id, admob_interstitial_id, admob_rewarded_id
     *    with your real Ad Unit IDs from AdMob dashboard.
     */
    private fun initAdsSafely() {
        try {
            MobileAds.initialize(this) { initStatus ->
                // This callback runs on a background thread — post to main thread
                runOnUiThread {
            ...
            // AdMob init failed — app continues without ads
        }
    }
    ...
    private fun showRewardedAd() {
        try {
            ...
            if (ad != null && !isFinishing && !isDestroyed) {
                ...
                Toast.makeText(this, "Ad not ready yet. Please try again shortly.", Toast.LENGTH_SHORT).show()
                loadRewardedAd()
            }
        } catch (e: Exception) {
    ...
    private fun setupClickListeners() {
        binding.btnGenerateSeo.setOnClickListener {
            ...
            updateLoadingText("🤖 Generating SEO content...")

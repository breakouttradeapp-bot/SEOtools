...
class SeoResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        else @Suppress("DEPRECATION") intent.getParcelableExtra(Constants.EXTRA_SEO_CONTENT)
        ...
        if (seoContent != null) { bindContent(seoContent); animateCards() }
        else showEmptyState()
    ...
    private fun bindContent(seo: SeoContent) {
        ...
        val full = buildString {
            ...
            if (seo.tags.isNotEmpty()) append("Tags:\n${seo.tags}\n\n")
            if (seo.hashtags.isNotEmpty()) append("Hashtags:\n${seo.hashtags}")
        }
        binding.btnShareAll.setOnClickListener {
            if (full.isNotBlank()) shareText(full)
    ...
    private fun showEmptyState() {
        ...
        binding.tvTags.text = "—"; binding.tvHashtags.text = "—"
    }

    private fun animateCards() {
        try {
            binding.cardTitle.animateIn(100); binding.cardDescription.animateIn(200)
            binding.cardTags.animateIn(300); binding.cardHashtags.animateIn(400)
        } catch (e: Exception) {}
    }
